...
public class Principal {
    ...
    public static void main(String[] args) throws JAXBException, IOException {
        ...
        for (int i = 0; i < empresa2.getEmpleados().toArray().length; i++) {
            System.out.println("Empleado " + (i + 1) + ": " + ((Empleado) empresa2.getEmpleados().get(i)).getNombre()
                    + " con DNI " + ((Empleado) empresa2.getEmpleados().get(i)).getDni() + " y "
                    + ((Empleado) empresa2.getEmpleados().get(i)).getEdad() + " años");
        }
    }
}
