...
public class App {
    public static void main(String[] args) throws URISyntaxException, IOException, InterruptedException {
        ...
        String[] ips = { "192.168.7.21", "192.168.7.17", "192.168.7.13",
                "192.168.7.9", "192.168.7.5", "192.168.7.1",
                "192.168.7.18", "192.168.7.14", "192.168.7.10", "192.168.7.2", "192.168.7.6",
                "192.168.7.11",
                "192.168.7.15", "192.168.7.19", "192.168.7.3", "192.168.7.20",
                "192.168.7.16", "192.168.7.8",
                "192.168.7.4" };

        /* String[] ips = { "localhost", "127.0.0.1", "192.168.7.13" }; */
        play(ips);
    }

    public static String consumeAPI(String ip, String path)
    ...
    public static int calcularPuntuacion(int iteration) {
        return 10 / iteration;
    }
    ...
    public static void play(String[] ips) throws URISyntaxException, IOException, InterruptedException {
            ...
            try {
                ...
                if (Boolean.parseBoolean(consumeAPI(ip, ""))) {
                    for (int i = 1; i < 4; i++) {
                        ...
                        System.out.println("Quieres adivinar el personaje? s/n");
                        ...
                        if (respuesta.equalsIgnoreCase("s")) {
                            ...
                            if (Boolean.parseBoolean(consumeAPI(ip, "resolver/" + respuesta)))
                                puntuacion += calcularPuntuacion(i);
                            numApis++;
                            break;
                        }
        ...
        try {
            int puntuacion_anterior = Integer.parseInt(consumeAPI("localhost", "score"));
            if (puntuacion > puntuacion_anterior)
                consumeAPI("localhost", "score/percebeiro10/" + puntuacion);
        } catch (Exception e) {
            System.out.println("Api no disponible");
            System.out.println("Error" + e);
        }
